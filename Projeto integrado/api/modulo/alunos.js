...
const getListAlunos = function(){
    let listAlunos = [];
    erro = true;

    alunos.forEach(item => {
        listAlunos.push(item)
            erro = false;
    });
    if(erro)
        return false;
...
//Get dos alunos com base na matrícula
const getAlunosMatricula = function (code){
    let matricula = code.toLowerCase();
    let alunosMatricula = {};
    let nomeAlunos = [];
    let erro = true;
...
//Get dos alunos com base no curso 
const getAlunosCurso = function (code){
    let curso = code.toLowerCase();
    let alunosCurso = {};
    let nomeAlunos = [];
    let erro = true;
...
//Get dos alunos com base no ano de conclusão
const getAlunosAnoConclusao = function (code){
    let anoConclusao = code.toLowerCase();
    let alunosAnoConclusao = {};
    let nomeAlunos = [];
    let erro = true;
    ...
    if(anoConclusao != undefined) {
        if (anoConclusao != ''){
            alunos.forEach(item => {
                 item.curso.forEach(item2 => {
                    if(item2.conclusao.toLowerCase().includes(anoConclusao)){
                        nomeAlunos.push(item.nome)
                        erro = false
                    }
                })
            });
            alunosAnoConclusao.alunos = nomeAlunos;
        }
    }
    if(erro)
        return false;
    else
        return alunosAnoConclusao;
}

//Get das disciplinas e suas respectivas médias com base na matricula
const getDisciplinasMedias = function (code){
    let matricula = code.toLowerCase();
    let disciplinasMedias = {};
    let alunosMatricula = [];
    let erro = true;
    ...
    if(matricula != undefined) {
        if (matricula != ''){
            ...
            disciplinasMedias.disciplinas = alunosMatricula
        }
    }
    if(erro)
        return false;
...
// Get dos alunos com base no status
const getAlunosStatus = function (code){
    ...
    if( status != undefined) {
        if (status != ''){
            alunos.forEach(item => {
                ...
                alunosStatus.alunos = nomeAlunos;
            });
        }
    }
    if(erro)
